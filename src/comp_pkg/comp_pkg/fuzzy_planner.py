...
class FuzzyPathPlanner(Node):
    def __init__(self, num_waypoints=1000,waypoint_tolerance=0.4):
        ...
        # Subscriptions
        self.create_subscription(Path, '/global_path_nav2', self.plan_callback, 10)
        self.create_subscription(Odometry, '/odom', self.odom_callback, 10)
        self.create_subscription(Point, '/closest_obstacle_in_range', self.closest_obstacle_callback, 10)
    ...
    def follow_waypoints(self):
        ...
        if self.current_wp_index >= len(self.waypoints):
            ...
            # Reset so a new path can be loaded next time
            self.waypoints = None
            self.current_wp_index = 0
    ...
    def parameter_callback(self, params):
        ...
        if ok:
            self.get_logger().info(
                f"Updated params → attractive_fuzzy_const={self.attractive_fuzzy_const},")
    ...
    def attractive_potential_fuzzy(self):
        ...
        c_1=(0.6-0)/(n_1-1)
        #c_1=(3-0)/(n_1-1)
        R_1=[0,0.2,0.3,0.4,0.5,0.52,0.55,0.6]
        a=0
        b=0
        for jj in range(n_1):
            if jj==0:
        ...
        d_1=b/a
        F_att = -d_1*diff*self.attractive_fuzzy_const*self.max_v_leader/np.linalg.norm(diff) # Attractive force vector
        return F_att
    ...
    def dynamic_repulsive_potential_fuzzy(self):
        ...
        for i in range(num_obs): # in this loop we will be calculating cos_delta and threat level to use in repulsive potential
            ...
            if dist_relative<=r_activate_p:
                ...
                R_2=[100,40,1,1.0,1,1,0.9,0.8,0.7,0]
                a=0
                b=0
                for jj in range(n_2):
                    if jj==0:
    ...
    def fuzzy_planner(self):
        ...
        self.get_logger().info(f"F_att_x={F_att[0][0]} , F_rep_x={F_rep[0][0]} , F_att_y={F_att[0][1]} ,F_rep_y={F_rep[0][1]} ,")
...
#ros2 param set /fuzzy_planner attractive_fuzzy_const 30.0
#ros2 param set /fuzzy_planner repulsive_fuzzy_const 15.0
